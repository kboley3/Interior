...
def adiabatic_atm(T_batm, g_batm, P_batm, r_batm, n):
    
    '''Usage: H, d_atm, m_atm = adiabatic_atm(T_atm, g_batm, P_batm, r_batm)
       
        T_atm -- temp assumed for isothermal atmosphere in K
        g_batm -- surface gravity  in m/s^2
        P_batm -- surface pressure in bars
        r_batm -- surface radius in m
        n --number of modes, 5 for diatomic
    ...
    An adiabatic atmosphere is assumed. This approximation is only valid for an
    atmospheric mass fraction (m_env/M_planet) of ~<0.001 (per Dorn et al. 2017).
    ...
    Pout = 0.02                     #20 mbar per Dorn et al. 2017 -- See how valid this is...
    bar2pa = 100000                 #conversion from bars to pascals, in case the input pressure
                                    #is in bars.
    mu = 0.0288                     #rough estimate of mean molecular weight in kg/mol
    R = 8.3144598                   #universal gas constant in J/mol*K
    ...
    Tout = np.power((Pout/P_batm),kappa)*T_batm
    
    cp = (n/2 + 1)*R/mu
    
    d_atm = (cp/g_batm)*(T_batm - Tout)
    
    H = T_batm*R/(g_batm*mu)
