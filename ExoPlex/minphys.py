...
def get_rho(Planet,grids,Core_wt_per,layers):
    ...
    #for i in range(len(rho_layers)):
        #print i,rho_layers[i],Pressure_layers[i]/1e4
    return rho_layers
...
def get_radius(Planet,layers):
    ...
    """
    if len(mass)<=num_core_layers:

        rhofunc = interpolate.UnivariateSpline(mass, density)
        radius_of_sphere = lambda p, x: ((x/rhofunc)*(3./(4.*np.pi)))^(1./3.)

        mass = np.ravel(odeint(radius_of_sphere,0.,mass))
        return mass

    elif len(mass) > (num_core_layers+num_mantle_layers):
        print "in here"
        mass_core = mass[:num_core_layers]
        density_core = density[:num_core_layers]
        rhofunc_core = interpolate.UnivariateSpline(mass_core, density_core)
        radius_in_core = lambda p, x: pow((4.*np.pi*rhofunc_core(x)),-1./3.)*pow(3*x,-2./3.)

        radius_core = np.ravel(odeint(radius_in_core, 0., mass_core))

        mass_mantle = mass[num_core_layers:num_core_layers + num_mantle_layers]
        density_mantle = density[num_core_layers:num_core_layers + num_mantle_layers]
        rhofunc_mantle = interpolate.UnivariateSpline(mass_mantle, density_mantle)
        radius_in_mantle = lambda p, x: pow((4.*np.pi*rhofunc_mantle(x)),-1./3.)*pow(3*x,-2./3.)

        radius_mantle = np.ravel(odeint(radius_in_mantle,radius_core[-1],mass_mantle))

        mass_water = mass[num_core_layers + num_mantle_layers:]
        density_water = density[num_core_layers + num_mantle_layers:]
        rhofunc_water = interpolate.UnivariateSpline(mass_water, density_water)

        radius_in_water = lambda p, x:  pow((4.*np.pi*rhofunc_water(x)),-1./3.)*pow(3*x,-2./3.)

        radius_water= np.ravel(odeint(radius_in_water, radius_mantle[-1], mass_water))

        return np.concatenate((radius_core,radius_mantle,radius_water),axis=0)
    else:
...
def get_temperature(Planet,grids,structural_parameters,layers):
    ...
    #if number_h2o_layers > 0:
        #init_grad = np.ravel(odeint(adiabat_mantle, 0.,depths_water[::-1]))[-1]
        #print init_grad
        #gradient_mantle = np.ravel(odeint(adiabat_mantle, init_grad,depths_mantle[::-1]))
    #else:
