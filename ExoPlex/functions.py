...
#constants, atomic masses
    ...
    #calculate FeMg from CMF input. Added by JGS.
    if args[9] !=0:
        FeMg = (args[1]/(1-args[1]))*(args[2]*(mSi+2*mO) + mMg+mO)/mFe
    else:
        FeMg = args[1]
        
    print ('FeMg test test: ', FeMg)
...
############################################################################################
    ...
    print ("FeO",FeO_mant_wt)
    print ("MgO",MgO_mant_wt)
    print ("SiO2",SiO2_mant_wt)
    print ("CaO",CaO_mant_wt)
    print ("Al2O3",Al2O3_mant_wt)
    print
    ...
    print Fe_core_wt
    print S_core_wt
    S_core_wt = round(S_core_wt,2)
    Fe_core_wt = round(Fe_core_wt,2)
    print Fe_core_wt
    print S_core_wt
    '''
...
def find_Planet_radius(radius_planet, core_mass_frac, structure_params, compositional_params, grids, Core_wt_per, layers):
    ...
    if layers[2] > 0:
        ...
        structure_params[6] = brentq(calc_CRF,.30,.75,args=args,xtol=1e-4)
        Planet = planet.initialize_by_radius(*[radius_planet, structure_params, compositional_params, layers])
        Planet = planet.compress_radius(*[Planet, grids, Core_wt_per, structure_params, layers])
...
def find_Planet_mass(mass_planet, core_mass_frac, structure_params, compositional_params, grids, Core_wt_per, layers, CMF = -1.0):

    import planet
    import minphys
    """
    def calc_mass(value, args):
        mass_to_fit = args[0]
        structure_params = args[1]
        compositional_params = args[2]
        layers = args[3]
        ...
        Planet = planet.initialize_by_mass(*[mass_to_fit, structure_params, compositional_params, layers],value)
        Planet = planet.compress(*[Planet, grids, Core_wt_per, structure_params, layers])

        planet_mass = minphys.get_mass(Planet,layers)

        print "Diff in Core Mass Fraction = ", '%.3e' % (mass_to_fit - planet_mass)
        return (mass_to_fit - planet_mass)
    """
    from scipy.optimize import brentq
    #args = [mass_planet, structure_params, compositional_params, layers,grids,Core_wt_per]
    #structure_params[6] = brentq(calc_mass,0.5,1.4,args=args,xtol=1e-4)
    Planet = planet.initialize_by_mass(*[mass_planet, structure_params, compositional_params, layers,core_mass_frac, CMF])

    #print "radius bef",Planet['radius'][-1]/1000.
    
    Planet = planet.compress_mass(*[Planet, grids, Core_wt_per, structure_params, layers])
    #print "mass",Planet['mass'][-1]/5.97e24


    print ("radius after",Planet['radius'][-1]/ 6371e3)
    return Planet
